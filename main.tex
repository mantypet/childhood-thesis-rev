...
\keywords{Lineaarinen sekamalli, rekisteriaineisto, FinLapset, Terveyden ja hyvinvoinnin laitos}
\depositeplace{}
...
Tutkielmassa arvioimme lineaaristen sekamallien soveltuvuutta FinLapset-rekisteriaineistosta muodostetun pitkittäisaineiston analyysivälineeksi. Teoriaosassa esittelemme lineaaristen sekamallien keskeiset ominaisuudet, estimointimenetelmät ja tarkastelemme hyviä mallinarvioinnin käytäntöjä. Soveltavassa vaiheessa sovitamme aineistoon kaksitasoisen lineaarisen sekamallin, jolla tutkimme lasten ja nuorten painoindeksin yhteyttä ikään ja biologiseen sukupuoleen sekä arvioimme mallin kykyä selittää aineistossa esiintyvää yksilökohtaista ja yksilöiden välistä painoindeksin vaihtelua. Mallin suoriutumista tarkastellaan erityisesti rekisteriaineiston analyysin haasteiden näkökulmasta.\\

Lineaarinen sekamalli muodostaa luontevan analyysityökalun FinLapset-rekisteriaineiston kaltaisen pitkittäisaineiston analyysiin. Yksinään iän kiinteä populaatiovaikutus, yhdessä yksilö- ja ikäkohtaisten satunnaivaikutusten kanssa selittää mallin vaihtelua erittäin hyvin. Painoindeksin ja iän yhteyden lineaarisuusoletus jää kuitenkin epäilyksen alaiseksi ja yksilökohtaisten residuaalien autokorrelaatio sekä varianssin heteroskedastisuus osoittautuvat merkittäviksi haasteiksi, joihin ei kyetä tyydyttävästi vastaamaan. Myös rekisteriaineistolle tyypilliset ominaisuudet, kuten passiivisesta kertymistavasta seuraava hyvin määritellyn populaatiokehikon puute ja aineistoa tuottavien prosessien osittainen tai täysi tuntemattomuus vaikeuttavat mallin estimaatteihin liittyvien epävarmuustekijöiden esittämistä. Suuresta havaintomäärästä seuraten estimaattorit ovat hyvin tarkkoja, mikä osaltaan antaa virheellisen kuvan mallin hyvyydestä, vaikka niihin harha jää osin tunnistamatta.\\

Tutkielmassa näytetään, että lineaarisille sekamallelle löytyy joustavia laajennoksia, joilla osa mallin haasteista voitaisiin ylittää. Myös erilaisia näkökulmia rekisteriaineiston erityispiirteiden huomioimiseksi ja pahimpien sudenkuoppien välttämiseksi punnitaan.\\
...
Luvussa~\ref{sec:pitkittaistutkimus} tarkastelemme pitkittäistutkimusta tutkimusasetelmana ja esittelemme tutkielman aineiston. Luvussa~\ref{sec:lsm} muodostamme teoreettisen pohjan tutkielman aineiston analyysissä käytettävälle lineaariselle sekamallille. Luvun~\ref{sec:lsmanalyysi} keskiössä on tutkielman aineiston analyysiin sopivan lineaarisen sekamallin muodostaminen sekä lopullisen mallin arviointi. Lopuksi, luvussa~\ref{sec:johto} tuomme yhteen tutkielman keskeiset havainnot ja johtopäätökset.\\
...
FinLapset-rekisterihankkeen keskeisiä tutkimuskysymyksiä olivat lasten ja nuorten ylipaino ja lihavuus sekä pituus- ja painotietojen valtakunnallinen kattavuus \citep{finlapsetrekisteri}. Tutkielman aineistoa hyödynnämme Terveyden ja hyvinvoinnin laitoksen luvalla. Tarkastelemme seuraavaksi aineiston keskeisiä rajauksia ja teknisiä määrityksiä. \\
...
Tulokset julkaistiin vuosilta 2014\textendash2018 sellaisilta lapsilta ja nuorilta, jotka olivat mittaushetkellä 2\textendash16-vuotiaita. \citep{finlapsetrekisteri}. \\
...
\vdots & \ddots & \vdots \\
\text{Cov}(\text{b}_{0i}, \text{b}_{qi}) & \dots & \text{Var}(\text{b}_{qi}) \\
\end{bmatrix}. \\
$$
...
Muodostetaan profiloitu log-uskottavuusfunktio $l(\bm{\theta} ; \bm{y})$ sijoittamalla $\hat{\bm{\beta}}$ log-uskottavuus\-funktioon $l(\bm{\beta}, \bm{\theta};\bm{y})$
...
Keskeinen ero on kuitenkin yksilöiden välinen satunnaisvaikutus $b_{i0}$, joka kiinteiden vaikutusten mallissa kuvaa yksilöiden vakiotermin vaihtelua. Koska $\bm{b}_i \sim N(0,\bm{G})$, ainoastaan vakiotermin vaihtelun sisältävän mallimme satunnaisvaikutusten kovarianssimatriisi on siten
...
Kappaleessa~\ref{sub:kovarianssirakenteet} totesimme, että matriisiin $\bm{R}_i$ voidaan korreloimattoman diagonaalirakenteen sijaan sisällyttää myös muita rakenteita. \cite{pinheiro00} mukaan tämä käytännössä vaatii kuitenkin lineaarisen sekamallin laajentamista.\\
...
\internalappendix{1}{Tutkielman työkalut}
\label{appx:tools}
...
\hline \\[-1.8ex] 
 ...
 Vakiotermi ($\hat{\beta_0}$) & 14,432802 & 14,439736 & 14,432899\\ 
  ...
  & & &\\ 
 ika\_i0 ($\hat{\beta_1}$) & $-$0,171405 & $-$0,172975 & $-$0,171421\\ 
